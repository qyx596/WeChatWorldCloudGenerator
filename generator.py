...
def getChat2csv(wechat_id_encode, DATABASE_PATH=DATABASE_PATH):
    try:
        ...
        cleandata = chat[
            (chat['Des'] == 0) & (~chat['Message'].str.contains('<roomid>')) & (~chat['Message'].str.contains('<msg>'))]
...
def csvProcess(CSV_PATH=CSV_PATH, WORDLIST_PATH=WORDLIST_PATH):
    ...
    try:
        with open(CSV_PATH, 'r') as f:
            content = f.read()
        remove_words = ["，", "!", "。", '\n', '\xa0', "我", "你", "的", "了", "就", "吗", "嗯", "哈", "不", "不是", "现在", "然后", "啊",
                        "是", "要", "没", "又", "那", "也", \
                        "有", "想", "去", "这", "个", "还", "都", "会", "啥", "吧", "说", "真", "呢", "呀", "得", "到", "看", "啦", "咋",
                        "老", "什么", "他们", "别", "给", "刚刚", "哼", "跟", "能", "过",
                        "一", "人", "他"]  # 移除常用语气词
